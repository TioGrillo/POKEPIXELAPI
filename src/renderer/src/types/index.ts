export interface SellConfig {
  ...
  // Sniper / Filtro de raridade
  sniper_enabled?: boolean;
  sniper_min_rarity?: string;   // ex: "uncommon" | "rare" | "epic" | "legendary"
  sniper_min_iv?: number;       // total IV mínimo (0-100)
  sniper_only_shiny?: boolean;  // capturar somente shinies
  [key: string]: any;
}
...
export interface AccountConfig {
  ...
  auto_potion_item_id?: string;
  auto_revive: boolean;
  auto_revive_item_id?: string;
  auto_sell_loot?: boolean;
  sell_pokemon: boolean;
  email?: string;
  password?: string;
  hunt_shiny_enabled?: boolean;
  hunt_min_quality?: string;
  hunt_min_iv?: number;
  auto_choose_zone?: boolean;
...
export const DEFAULT_SELL_CONFIG: SellConfig = {
  ...
  sniper_min_rarity: "uncommon",
  sniper_min_iv: 0,
  sniper_only_shiny: false,
};
...
export function createDefaultAccount(name: string, token: string, hunt: string): AccountConfig {
  return {
    ...
    auto_sell_loot: true,
    sell_pokemon: false,
    auto_choose_zone: false,
    auto_buy: {
      enabled: false,
      min_gold: 1000,
      balls: [
        { item_id: "capsule_ultra", enabled: true, min_qty: 20, max_qty: 100 },
        { item_id: "capsule_master", enabled: false, min_qty: 5, max_qty: 20 },
      ],
      potions: [
        { item_id: "potion_great", enabled: true, min_qty: 10, max_qty: 50 },
      ],
      revives: [
        { item_id: "revive_basic", enabled: true, min_qty: 5, max_qty: 20 },
      ],
    },
  };
}
